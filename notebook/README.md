...
cargo test -p notebook   # 跑 30 个测试（单元 + 集成 + doctest）
```
...
| 概念 | 在本项目里的体现（一个方法一个） |
|------|----------------------------------|
| **结构体与 `impl` 块** | `struct Entry` / `struct Notebook` 只声明字段，行为全挂在 `impl Notebook` 上；`new()` 是关联函数，其余都是方法 |
| **字段可见性** | `Entry` 的字段全 `pub`，`Notebook.entries` 私有——外面只能走方法，所以底层容器随时能换 |
...
对应笔记：[05 结构体与方法](../docs/05-structs-and-methods.md)、
[03 所有权与借用](../docs/03-ownership-and-borrowing.md)、
[11 生命周期（深入所有权）](../docs/11-lifetimes-and-more-ownership.md)。
...
- **`get` 的借用和 `get_mut` 冲不冲突，看的是「后面还用不用」，不是「声明了没」。**
  ...
  `borrow_ends_at_last_use` 钉着，展开讲在[笔记 11](../docs/11-lifetimes-and-more-ownership.md#坑)。
...
src/main.rs          读命令 -> 打印结果，I/O 只在最外层（复习 docs/10）
tests/public_api.rs  集成测试：独立 crate，只看得见 pub 的东西
```

输入和输出都是 `run` 的参数，所以**整段 REPL 交互**都能在测试里跑：喂一个 `&[u8]`，
用 `Vec<u8>` 接住它说过的每句话，再断言。这是[笔记 10](../docs/10-testing.md) 那三层
可测性设计在本项目里的落法。
